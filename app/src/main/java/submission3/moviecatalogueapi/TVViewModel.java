...
public class TVViewModel extends ViewModel {
    ...
    private MutableLiveData<ArrayList<TVModel>> listData = new MutableLiveData<>();

    void setData(final String language) {
        AsyncHttpClient client = new AsyncHttpClient();
        final ArrayList<TVModel> listItems = new ArrayList<>();
            ...
            public void onSuccess(int statusCode, Header[] headers, byte[] responseBody) {
                try {
                    ...
                    for (int i = 0; i < jsonArray.length(); i++) {
                        ...
                        TVModel tvModel = new TVModel(jsonObject);
                        listItems.add(tvModel);
                    }
    ...
    LiveData<ArrayList<TVModel>> getData() {
        return listData;
    }
}
